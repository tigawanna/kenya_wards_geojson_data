...
describe("SQLite Triggers", () => {
  ...
  it("should have ward triggers registered", () => {
    const triggers = db
      .prepare("SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE '%ward%'")
      .all();
  ...
  it("should fire INSERT trigger when ward is added", () => {
    ...
    const initialCount = db.prepare("SELECT COUNT(*) as count FROM kenya_ward_events").get() as {
      count: number;
    };
    ...
    const afterCount = db.prepare("SELECT COUNT(*) as count FROM kenya_ward_events").get() as {
      count: number;
    };
